...
bool ELFImage::DeleteUseless()
{
	...
	newBuffSize = lastPh.p_offset+lastPh.p_filesz;
	
	if (FileHeader()->e_shoff >= newBuffSize) 
	{
...
void ELFImage::BuildFile(const char* name)
{
	...
	// ofs.write((char*)&m_eh,sizeof(m_eh));
	// for(auto &ph:m_phTable)
	// {
	// 	ofs.write((char*)&ph,sizeof(ph));
	// }
	// char* zeroBuf = new char[40960];
	// memset(zeroBuf,0,1024);
	// for(auto &sec:m_secTable)
	// {
	// 	if(sec.header.sh_type == SHT_NOBITS) continue;
	// 	size_t pos = ofs.tellp();

	// 	if(pos < sec.header.sh_offset)
	// 	{
	// 		size_t padding = sec.header.sh_offset-pos;
	// 		ofs.write(zeroBuf,padding);
	// 	}
	// 	ofs.write((char*)sec.data,sec.header.sh_size);
	// }
	// for(auto &sec:m_secTable)
	// {
	// 	ofs.write((char*)&sec,sizeof(sec));
	// }
